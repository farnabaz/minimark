...
function getRows(element: MinimarkNode): MinimarkElement[] {
...
function getCells(row: MinimarkElement): MinimarkElement[] {
  const [, , ...children] = row
  return children.filter(child =>
...
export function table(node: MinimarkElement, state: State) {
  const [, , ...children] = node

  // Extract thead and tbody
  let headerRows: MinimarkElement[] = []
  let bodyRows: MinimarkElement[] = []
  ...
  if (headerRows.length === 0 && bodyRows.length > 0) {
    ...
    const headerCells: MinimarkElement[] = cells.map((_, i) =>
      ['th', {}, `Column ${i + 1}`] as MinimarkElement,
    )
    headerRows = [['tr', {}, ...headerCells] as MinimarkElement]
  }
...
export function thead(_node: MinimarkElement, _state: State) {
  // thead is handled by the table handler
  return ''
}

export function tbody(_node: MinimarkElement, _state: State) {
  // tbody is handled by the table handler
  return ''
}

export function tr(_node: MinimarkElement, _state: State) {
  // tr is handled by the table handler
  return ''
}

export function th(_node: MinimarkElement, _state: State) {
  // th is handled by the table handler
  return ''
}

export function td(_node: MinimarkElement, _state: State) {
